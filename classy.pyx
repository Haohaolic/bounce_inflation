    ...
    def get_current_derived_parameters(self, names):
        ...
        for name in names:
            if name == 'h':
                ...
                self.compute(["fourier"])
                if (self.pt.has_pk_matter == _FALSE_):
                    raise CosmoSevereError("No power spectrum computed. In order to get S_8, you must add mPk to the list of outputs.")
                value = self.fo.sigma8[self.fo.index_pk_m]*np.sqrt(self.ba.Omega0_m/0.3)
 #           elif name == 'S8':
 #               value == self.fo.S8
            elif name == 'k_eq':
